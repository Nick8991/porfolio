  ...
  <div class="container" :style="{ 'background-image': 'url(' + image + ')' }">
    <div
      ...
      <div class="project_desc w-full sm:w-[45%]">
        ...
        <div class="techs flex flex-col my-7">
          ...
          <div class="technos_container flex space-x-5">
            <img class="w-[35px]" :src="tech.image" :alt="tech.name" :title="tech.name"
              v-for="(tech, idx) in used_technos" :key="idx" />
          </div>
        </div>
          ...
          <a :href="live_link">
            <button
              ...
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M5 15L15 5" stroke="" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M6.875 5L15 5L15 13.125" stroke="" stroke-width="2" stroke-linecap="round"
                  stroke-linejoin="round" />
              </svg>
            </button>
          </a>
          <a :href="git_link" target="_blank"  :class="{ 'hidden': git_link.length == 0 }">
      ...
      <div class="project_image w-full sm:w-[50%] relative hidden sm:block">
        <img :src="image" alt="" />
        <div class="absolute top-0 bottom-0 left-0 right-0 w-full bg-important opacity-30 hover:opacity-0 transition-all">
        </div>
      </div>
    </div>
  </div>
